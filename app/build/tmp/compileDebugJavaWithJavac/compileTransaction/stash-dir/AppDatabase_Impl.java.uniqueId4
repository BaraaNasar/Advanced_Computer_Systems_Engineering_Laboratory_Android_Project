...
import com.personal.finance.data.dao.BudgetDao_Impl;
import com.personal.finance.data.dao.CategoryDao;
import com.personal.finance.data.dao.CategoryDao_Impl;
import com.personal.finance.data.dao.TransactionDao;
import com.personal.finance.data.dao.TransactionDao_Impl;
import com.personal.finance.data.dao.UserDao;
import com.personal.finance.data.dao.UserDao_Impl;
...
public final class AppDatabase_Impl extends AppDatabase {
  ...
  protected SupportSQLiteOpenHelper createOpenHelper(DatabaseConfiguration configuration) {
    final SupportSQLiteOpenHelper.Callback _openCallback = new RoomOpenHelper(configuration, new RoomOpenHelper.Delegate(2) {
      ...
      public void createAllTables(SupportSQLiteDatabase _db) {
        _db.execSQL("CREATE TABLE IF NOT EXISTS `users` (`email` TEXT NOT NULL, `firstName` TEXT, `lastName` TEXT, `password` TEXT, PRIMARY KEY(`email`))");
        _db.execSQL("CREATE TABLE IF NOT EXISTS `transactions` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `amount` REAL NOT NULL, `date` INTEGER NOT NULL, `category` TEXT, `description` TEXT, `type` TEXT, `userEmail` TEXT, FOREIGN KEY(`userEmail`) REFERENCES `users`(`email`) ON UPDATE NO ACTION ON DELETE CASCADE )");
        _db.execSQL("CREATE INDEX IF NOT EXISTS `index_transactions_userEmail` ON `transactions` (`userEmail`)");
        _db.execSQL("CREATE TABLE IF NOT EXISTS `budgets` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `category` TEXT, `limitAmount` REAL NOT NULL, `userEmail` TEXT, FOREIGN KEY(`userEmail`) REFERENCES `users`(`email`) ON UPDATE NO ACTION ON DELETE CASCADE )");
        _db.execSQL("CREATE INDEX IF NOT EXISTS `index_budgets_userEmail` ON `budgets` (`userEmail`)");
        _db.execSQL("CREATE TABLE IF NOT EXISTS `categories` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `name` TEXT, `type` TEXT, `userEmail` TEXT)");
        _db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS `index_categories_name_type_userEmail` ON `categories` (`name`, `type`, `userEmail`)");
        _db.execSQL("CREATE TABLE IF NOT EXISTS room_master_table (id INTEGER PRIMARY KEY,identity_hash TEXT)");
        _db.execSQL("INSERT OR REPLACE INTO room_master_table (id,identity_hash) VALUES(42, 'f5afa33450f18dd80e1a2bbabcc04671')");
      }
      ...
      public RoomOpenHelper.ValidationResult onValidateSchema(SupportSQLiteDatabase _db) {
        ...
        if (! _infoUsers.equals(_existingUsers)) {
          return new RoomOpenHelper.ValidationResult(false, "users(com.personal.finance.data.model.User).\n"
                  + " Expected:\n" + _infoUsers + "\n"
                  + " Found:\n" + _existingUsers);
        }
        ...
        _columnsTransactions.put("userEmail", new TableInfo.Column("userEmail", "TEXT", false, 0, null, TableInfo.CREATED_FROM_ENTITY));
        final HashSet<TableInfo.ForeignKey> _foreignKeysTransactions = new HashSet<TableInfo.ForeignKey>(1);
        _foreignKeysTransactions.add(new TableInfo.ForeignKey("users", "CASCADE", "NO ACTION",Arrays.asList("userEmail"), Arrays.asList("email")));
        final HashSet<TableInfo.Index> _indicesTransactions = new HashSet<TableInfo.Index>(1);
        _indicesTransactions.add(new TableInfo.Index("index_transactions_userEmail", false, Arrays.asList("userEmail"), Arrays.asList("ASC")));
        final TableInfo _infoTransactions = new TableInfo("transactions", _columnsTransactions, _foreignKeysTransactions, _indicesTransactions);
        final TableInfo _existingTransactions = TableInfo.read(_db, "transactions");
        if (! _infoTransactions.equals(_existingTransactions)) {
          return new RoomOpenHelper.ValidationResult(false, "transactions(com.personal.finance.data.model.Transaction).\n"
                  + " Expected:\n" + _infoTransactions + "\n"
        ...
        _columnsBudgets.put("userEmail", new TableInfo.Column("userEmail", "TEXT", false, 0, null, TableInfo.CREATED_FROM_ENTITY));
        final HashSet<TableInfo.ForeignKey> _foreignKeysBudgets = new HashSet<TableInfo.ForeignKey>(1);
        _foreignKeysBudgets.add(new TableInfo.ForeignKey("users", "CASCADE", "NO ACTION",Arrays.asList("userEmail"), Arrays.asList("email")));
        final HashSet<TableInfo.Index> _indicesBudgets = new HashSet<TableInfo.Index>(1);
        _indicesBudgets.add(new TableInfo.Index("index_budgets_userEmail", false, Arrays.asList("userEmail"), Arrays.asList("ASC")));
        ...
        if (! _infoBudgets.equals(_existingBudgets)) {
          return new RoomOpenHelper.ValidationResult(false, "budgets(com.personal.finance.data.model.Budget).\n"
                  + " Expected:\n" + _infoBudgets + "\n"
                  + " Found:\n" + _existingBudgets);
        }
        ...
        _columnsCategories.put("userEmail", new TableInfo.Column("userEmail", "TEXT", false, 0, null, TableInfo.CREATED_FROM_ENTITY));
        final HashSet<TableInfo.ForeignKey> _foreignKeysCategories = new HashSet<TableInfo.ForeignKey>(0);
        final HashSet<TableInfo.Index> _indicesCategories = new HashSet<TableInfo.Index>(1);
        _indicesCategories.add(new TableInfo.Index("index_categories_name_type_userEmail", true, Arrays.asList("name","type","userEmail"), Arrays.asList("ASC","ASC","ASC")));
        final TableInfo _infoCategories = new TableInfo("categories", _columnsCategories, _foreignKeysCategories, _indicesCategories);
        final TableInfo _existingCategories = TableInfo.read(_db, "categories");
        if (! _infoCategories.equals(_existingCategories)) {
          return new RoomOpenHelper.ValidationResult(false, "categories(com.personal.finance.data.model.Category).\n"
                  + " Expected:\n" + _infoCategories + "\n"
                  + " Found:\n" + _existingCategories);
        }
        return new RoomOpenHelper.ValidationResult(true, null);
      }
    }, "f5afa33450f18dd80e1a2bbabcc04671", "afa6ecd73d20c959eb40914e5ca7fd1a");
  ...
  protected InvalidationTracker createInvalidationTracker() {
    ...
    return new InvalidationTracker(this, _shadowTablesMap, _viewTables, "users","transactions","budgets","categories");
  }
